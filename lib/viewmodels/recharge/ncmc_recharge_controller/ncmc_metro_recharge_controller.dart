...
class NcmcRechargeController extends GetxController {
  final GlobalKey<FormState> formKey = GlobalKey<FormState>();
  final BillPayRepo repository = BillPayRepo();
  ...
  void onInit() {
    ...
    // Here we call API with NCMC category
    callBillerNameApi(serviceCategory: "NCMC Recharge");
  ...
  Future<void> callBillerNameApi({required String serviceCategory}) async {
    try {
      HealthInsuranceRequestModel data = HealthInsuranceRequestModel(
        billerCategory: serviceCategory,
  ...
  Future<void> fetchBillerDetails() async {
    ...
    try {
      ...
      CustomToast.show("Error fetching NCMC biller: $e");
    } finally {
      isLoading.value = false;
    }
  }
  ...
  void onClose() {
    ...
    if (Get.isRegistered<NcmcRechargeController>()) {
      Get.delete<NcmcRechargeController>();
    }
